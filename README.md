...
*Обозн.* y(t) ~ I(1), т.е. Δy(t)=y(t)-y(t-1) ~ I(0), где Δ - разностный оператор

Как происходит проверка на наличие единичных корней? Для получения ответа, сравниваем найденное с помощью функции `sm.tsa.adfuller(ts)` `p-value` - достоверность статистики с `critical_values` - критическимми значениями с уровнем значимости 5%:
* `p-value` > `sm.tsa.adfuller(ts)` => единичный корень есть и ряд не стационарен
* иначе стационарен
...
4. Поиск коэффициента интегрируемости ряда
	* `findOrder(ts, isPrint)` - возвращает коэффициент интегрируемости ряда
		+ ts - временной ряд
		+ isPrint - флаг: если True - печать в тесте Дики-Фуллера, если False - ничего не выводит
5. Логарифмирование ряда
	* `np.log(ts)` - поиск натурального логарифма от значений ряда
	* Зачем это нужно? Повышает точность предсказаний
	* Ряд является интегрированным порядка 1 => находим разность прологарифмированного ряда
6. Подбираем нужные параметры с помощью функции автокорреляции и функции частичной автокорреляции
